...
public class ImageUtil {
    ...
    public BufferedImage getBodyOutFile(BufferedImage image, InputStream inputStream) throws Exception {
        //调用百度api进行人像分割
    ...
    /**
     * 灰度图转换为纯黑白图
     * @param labelmapBase64
     * @param realWidth
     * @param realHeight
     * @return
     */
    private BufferedImage convert(String labelmapBase64, int realWidth, int realHeight) {
        try {
    ...
    public String bodySeg(InputStream inputStream) throws Exception {
        System.out.println("开始请求百度人像分割aip接口");
        ...
        System.out.println("请求结束，耗时(s): " + (System.currentTimeMillis() - start) / 1000F);
    ...
    private String getAuth() throws Exception {
        ...
        sb.append("?grant_type=client_credentials").append("&client_id=" + clientId);
        URL resUrl = new URL(sb.toString());
        //与resUrl建立网络连接
        HttpURLConnection connection = (HttpURLConnection) resUrl.openConnection();
        connection.setRequestMethod("GET");
        connection.connect();
        // 定义 BufferedReader输入流来读取响应数据
